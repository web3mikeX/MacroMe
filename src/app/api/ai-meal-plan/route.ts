...
export async function POST(request: NextRequest) {
  try {
    ...
    let enhancedPlan = basePlan
    let aiSuggestions: any[] = []
    let adaptedRecipes: any[] = []
    ...
    if (aiEnhancements.adaptRecipes || aiEnhancements.generateSuggestions) {
      ...
      const weeklyTargets = {
        ...
        protein: Math.round((userProfile.kcal_target * userProfile.protein_pct) / 100 / 4) * 7,
        carbs: Math.round((userProfile.kcal_target * userProfile.carb_pct) / 100 / 4) * 7,
        fat: Math.round((userProfile.kcal_target * userProfile.fat_pct) / 100 / 9) * 7,
      }

      // Get available pantry ingredients
      const availableIngredients = pantryItems?.map(item => item.ingredients.name) || []

      if (aiEnhancements.adaptRecipes && basePlan.missingIngredients.length > 0) {
        ...
        try {
          ...
          console.log(`✅ Successfully adapted ${adaptedRecipes.length} recipes`)
        } catch (error) {
            ...
            try {
              await supabase
                ...
                .insert({
                  ...
                  reasoning: 'Generated based on available pantry items and macro targets',
                  macro_targets: dailyTargets,
                  confidence_score: 0.8
                })
            } catch (error) {
              console.log('AI suggestions table not yet available, skipping storage:', error)
            }
          }

          console.log(`✅ Generated ${aiSuggestions.length} AI meal suggestions`)
        } catch (error) {
    ...
    if (aiEnhancements.optimizePantry && basePlan.missingIngredients.length > 0) {
    ...
    const response = {
      ...
      metadata: {
        ...
        aiConfidence: adaptedRecipes.length > 0 
          ? adaptedRecipes.reduce((sum, r) => sum + r.confidence, 0) / adaptedRecipes.length 
          : null
      }
    }
